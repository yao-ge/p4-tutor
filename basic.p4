...
control MyIngress(inout headers hdr,
    ...
    action scan_find() {
        ...
        hdr.udp.is_find = (hdr.udp.payload[15:0] == FIND_STRING) ? (bit<16>)255:(bit<16>)0;
        tmp_start = 16;
        start_add(tmp_start);
        bit<16> tmp = 1;
    ...
    //action determine_value(bit<256> val){
    action determine_value(bit<16> val){
        if(val == FIND_STRING)
        {
            hdr.udp.is_find = 255;
        }
    ...
    action init_value_length() {
        //bit<256> tmp_value = hdr.udp.payload;
        bit<16> tmp_value;
        bit<32> index = 0;
    ...
    action value_shift_scan() {
        ...
        determine_value(tmp_value);
        hdr.udp.is_find = (hdr.udp.is_find == 255) ? (bit<16>)255:(bit<16>)0;
        if(hdr.udp.is_find != 255){
